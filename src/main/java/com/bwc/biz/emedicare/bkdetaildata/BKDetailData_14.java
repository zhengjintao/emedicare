...
public class BKDetailData_14 {
	...
	private List<int[]> indexList = Arrays.asList(
		...
		new int[] {10, 1 },new int[] {10, 3 },
		new int[] {12, 1 }
	    );
	
	private List<String[]> lableList = Arrays.asList(
		new String[] { "1", "ID","ID"},new String[] { "2", "检查日期","检查日期"},
		new String[] { "3", "姓名","姓名"},new String[] { "4", "年龄/性别","年龄/性别"},
		new String[] { "5", "血氧饱和度","检查的结果"},new String[] { "6", "血氧饱和度","判定"},
		new String[] { "7", "判定区分","判定区分"},new String[] { "8", "说明事项","说明事项"},
		new String[] { "9", "说明","说明"}
		);

	public BKDetailData_14() {}

	/*
	 * 数据导入用构造函数
	 */
	public BKDetailData_14(Sheet sheet, String userid, String username, String date, String histno) {
		this.sheet = sheet;
		this.userid = userid;
	...
	public void saveDataExcelToDb() {
		...
		String context = "";
		// 插入用SQL
		String insertSql = "insert into cdata_detail_14 value(?,?,?,?,?,?,?,?)";
	...
	public List<String> getDateValue(String userid, String historydate, String historyno) {
		...
		String dataSql = "select context from cdata_detail_14 where userid = ? and examdate= ? and historyno= ? order by dispindex";
	...
	public void deleteData(String userid, String historydate){
		...
		params[0]= userid;
		params[1]= historydate;
		String delsql = "delete from cdata_detail_14 where userid = ? and examdate = ?";
	...
	private void check(String userid, String date) {
		...
		String checksql = "select * from cdata_detail_14 where userid = ? and examdate = ?";
