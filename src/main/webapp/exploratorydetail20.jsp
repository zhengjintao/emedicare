...
app.controller('ListController', function($scope,$http,transFormFactory) {
	var list = this;
	  ...
	  <%List<String> dataList = (List<String>) request.getAttribute("detailDataList20");%>;
	  <%for (int i = 0; i < dataList.size(); i++) {%>  
	       list.dt_20[<%=i%>] = "<%=StringUtil.javaScriptStringEnc(dataList.get(i))%>";
	   <%}%>
	 /*  (function(){
		  $scope.url =  "exploratorydetail.do";
		  var postdata = {'mode':'list', 'expid':list.expid, 'sheetid': '20'};
	      $http(
	  		{
	  			method:"POST",
	  			url:$scope.url,
...
<div class="ui bottom attached segment pushable">
  ...
  <tbody>
    ...
    <tr>
      <td colspan="4"><pre>{{list.dt_20[10]}}</pre></td>
    </tr>
  </tbody>
</table>
</div>
